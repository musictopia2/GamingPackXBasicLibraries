...
namespace BaseGPXWindowsAndControlsCore.BasicControls.GameFrames
{
    ...
    {
        public static readonly DependencyProperty HandTypeProperty = DependencyProperty.Register("HandType", typeof(HandViewModel<CA>.EnumHandList), typeof(BaseHandWPF<CA, GC, GW>), new FrameworkPropertyMetadata(HandViewModel<CA>.EnumHandList.Horizontal, new PropertyChangedCallback(HandTypePropertyChanged)));
        ...
        protected void RecalulateFrames() // try it this way.
        {
            ...
            if (whatCat == HandViewModel<CA>.EnumHandList.Horizontal)
            {
        ...
        protected void RecalculatePositioning() //needs to be protected so games like life card game can access it.
        {
            ...
            foreach (var thisCard in ObjectList!)
            {
                ...
                if (ThisDeck != null)
                {
                    ...
                    {
                        ThisDeck.Margin = new Thickness(0, 0, 0, 0);
                        SetLocations(ThisDeck);
                    }
                }
            }
        ...
        protected virtual void AfterCollectionChange()
        {
            if (ObjectList.Any(Items => Items.Drew == true))
            {
                ...
                if (HandType == HandViewModel<CA>.EnumHandList.Horizontal)
        ...
        public void LoadList(HandViewModel<CA> thisMod, string tagUsed) // don't need the cardlist because that is in the viewmodel
        {
            ...
            _tagUsed = tagUsed;
            DataContext = thisMod;
            SetBinding(TextProperty, nameof(HandViewModel<CA>.Text));
            SetBinding(IsEnabledProperty, nameof(HandViewModel<CA>.IsEnabled));
            _thisMod = thisMod;
        ...
        {
            SetBinding(VisibilityProperty, GetVisibleBinding(nameof(HandViewModel<CA>.Visible)));
        }
