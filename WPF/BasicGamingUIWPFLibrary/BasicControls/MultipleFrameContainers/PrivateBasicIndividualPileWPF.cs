...
namespace BasicGamingUIWPFLibrary.BasicControls.MultipleFrameContainers
{
        ...
        where GW : BaseDeckGraphicsWPF<CA, GC>, new()
    {
        ...
        public void Init(string tagUsed) //you do need to call in the tagused.
        {
            ...
            _thisGraphics.SetBinding(GraphicsCommand.CommandProperty, thisBind);
            _thisGraphics.CommandParameter = ThisPile; // this time, the parameter is the pile, not the card
            Binding binding = new Binding(nameof(BasicPileInfo<CA>.Visible));
            binding.Converter = new BooleanToVisibilityConverter();
            SetBinding(VisibilityProperty, binding); //hopefully no problem.
            //if i run into problems has to rethink.
            if (MainMod.HasFrame == true && MainMod.HasText == true)
            {
