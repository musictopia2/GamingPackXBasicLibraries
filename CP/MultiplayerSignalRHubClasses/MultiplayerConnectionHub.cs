...
namespace MultiplayerSignalRHubClasses
{
    public class MultiplayerConnectionHub : Hub
    {
        ...
        {
            ConnectionInfo thisCon = _playerList.Where(Items => Items.Value.ConnectionID == Context.ConnectionId).Select(Items => Items.Value).SingleOrDefault();
            if (thisCon != null)
        ...
        public async Task HostingAsync(string nickName) //send message only to user.
        {
            ...
            ConnectionInfo thisConnect = new ConnectionInfo();
            _hostName = nickName;
            thisConnect.ConnectionID = Context.ConnectionId;
            thisConnect.UserID = nickName;
            thisConnect.IsConnected = true;
            _playerList.TryAdd(nickName, thisConnect);
