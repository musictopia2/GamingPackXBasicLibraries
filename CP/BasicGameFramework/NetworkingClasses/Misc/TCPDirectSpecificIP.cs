...
namespace BasicGameFramework.NetworkingClasses.Misc
{
    public class TCPDirectSpecificIP : ObservableObject, IMessageChecker, INetworkMessages, IServerMessage
    {
        ...
        public TCPDirectSpecificIP(ITCPInfo thisTCP, IMessageProcessor thisMessage, IOpeningMessenger thisOpen)
        {
            _thisTCP = thisTCP;
        ...
        public async Task ProcessDataAsync(SentMessage thisData) //done.
        {
            ...
            if (thisData.Status == "clienthost")
            {
                if (thisData.Body != "")
                {
                    await _thisOpen.ConnectedToHostAsync(this, thisData.Body);
                    return;
                }
