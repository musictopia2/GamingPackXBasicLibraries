...
namespace BaseSolitaireClassesCP.ClockClasses
{
    public class ClockViewModel : ObservableObject
    {
        ...
        public ClockViewModel(IClockVM thisMod)
        {
            ThisMod = thisMod;
            _thisP = thisMod.MainContainer!.Resolve<IProportionImage>(ts.TagUsed);
            ClockCommand = new PlainCommand<SolitaireCard>(async thisCard =>
            {
                if (CurrentCard != null)
                {
                    if (CurrentCard.Deck == thisCard.Deck)
                    {
                ...
                {
                    if (thisClock.CardList.Last().Equals(thisCard))
                    {
            ...
            {
                return thisCard.IsEnabled == true && thisCard.CardType != EnumCardTypeList.Stop;
