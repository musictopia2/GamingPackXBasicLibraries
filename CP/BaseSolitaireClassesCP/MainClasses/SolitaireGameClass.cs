...
{
    public abstract class SolitaireGameClass<S> where S : SolitaireSavedClass, new()
    {
        ...
        public SolitaireGameClass(IBasicSolitaireVM thisMod) //you need the main view model loaded first or will have overflow errors.
        {
            _thisMod = thisMod;
            DeckList = new RegularCardsBasicShuffler<SolitaireCard>(); //i think this should be fine.
            _thisState = thisMod.MainContainer!.Resolve<ISaveSinglePlayerClass>(); //good news is did not.
            SaveRoot = new S(); //i think.
            SolitaireData1 = thisMod.MainContainer.Resolve<ISolitaireData>();
            LinkData();
        }
        private void LinkData()
        {
            ...
            _thisMod.MainPiles1!.IsRound = SolitaireData1.MainRound;
            _thisMod.WastePiles1!.HowManyPiles = SolitaireData1.WastePiles;
            _thisMod.MainPiles1.Rows = SolitaireData1.Rows;
            _thisMod.MainPiles1.Columns = SolitaireData1.Columns;
            if (SolitaireData1.WasteColumns > 0 && SolitaireData1.WasteRows > 0)
            {
                ...
                _thisMod.WastePiles1.FirstLoad(SolitaireData1.WasteRows, SolitaireData1.WasteColumns);
            }
            else if (SolitaireData1.WasteColumns == 0 && SolitaireData1.WasteRows == 0)
                _thisMod.WastePiles1.FirstLoad(SolitaireData1.IsKlondike, DeckList.ToRegularDeckDict());
            else
                throw new BasicBlankException("Either has to be 0 columns and 0 rows for the waste.  Or both has to be greater than 0");
            _thisMod.WastePiles1.CardsNeededToBegin = SolitaireData1.CardsNeededWasteBegin;
            _thisMod.MainPiles1.CardsNeededToBegin = SolitaireData1.CardsNeededMainBegin;
            _thisMod.MainPiles1.FirstLoad(SolitaireData1.SuitsNeedToMatchForMainPile, SolitaireData1.ShowNextNeededOnMain);
        }
        private bool _didInit;
        public async Task InitAsync() //another option open.
        {
        ...
        private void AfterInit()
        {
            ...
            _thisMod.DeckPile!.Visible = true;
            _thisMod.MainDiscardPile!.Visible = true; //hopefully does not have to use busy anymore (?)
        }
        ...
        protected async Task FinishAddingToMainPileAsync(int pile, SolitaireCard thisCard)
        {
            ...
            else if (_thisMod.DeckPile!.DeckStyle == DeckViewModel<SolitaireCard>.EnumStyleType.AlwaysKnown
        ...
        {
            await _thisMod.ShowGameMessageAsync("Congratulations, you won");
        ...
        {
            if (_thisMod.MainDiscardPile!.PileEmpty() == false && _thisMod.DeckPile!.DeckStyle == DeckViewModel<SolitaireCard>.EnumStyleType.Unknown)
                return;
            if (_thisMod.DeckPile!.IsEndOfDeck() && _thisMod.DeckPile.DeckStyle == DeckViewModel<SolitaireCard>.EnumStyleType.AlwaysKnown)
                return;
            SolitaireCard thisCard;
            if (_thisMod.DeckPile.DeckStyle == DeckViewModel<SolitaireCard>.EnumStyleType.Unknown)
        ...
        protected virtual SolitaireCard CardPlayed()
        {
            ...
            if (_thisMod.DeckPile!.DeckStyle == DeckViewModel<SolitaireCard>.EnumStyleType.AlwaysKnown)
            {
                if (_thisMod.DeckPile.IsEndOfDeck())
                    return new SolitaireCard();
