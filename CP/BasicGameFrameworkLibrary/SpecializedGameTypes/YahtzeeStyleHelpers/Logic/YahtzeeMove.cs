...
namespace BasicGameFrameworkLibrary.SpecializedGameTypes.YahtzeeStyleHelpers.Logic
{
        ...
        where D : SimpleDice, new()
    {
        ...
        public async Task MakeMoveAsync(RowInfo row)
        {
            ...
            {
                await UIPlatform.ShowMessageAsync($"Everyone gets their turns skipped except for {_gameContainer.SingleInfo.NickName}.  Also, everyone will get a 0 for the category closest to the top because {_gameContainer.SingleInfo.NickName} got a Kismet even though it was already marked");
            }
