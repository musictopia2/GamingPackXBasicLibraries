...
namespace BasicGameFrameworkLibrary.SpecializedGameTypes.StockClasses
{
        ...
        where D : IDeckObject, new()
    {
        ...
        public void AddToEmptyDiscard(D thisCard)
        {
            ...
            throw new BasicBlankException("There was no empty piles left.  Find out what happened");
        }
        ...
        {
            foreach (var ThisPile in PileList!)
                ThisPile.ObjectList.Clear();// you need to clear all the cards in each pile.
        }
        ...
        {
            newDiscard = new DiscardPilesVM<D>(_command, _aggregator);
        ...
        public DiscardPilesVM(CommandContainer command, IEventAggregator aggregator) : base(command, aggregator)
        {
            _command = command;
