    ...
    public class GamePackageDIContainer : IGamePackageResolver, IGamePackageRegister, IGamePackageDIContainer
    {
        ...
        private void SetResults(ContainerData thisResults, string tag)
        {
            if (_thisSet.Any(x => x.TypeOut == thisResults.TypeOut && x.Tag == tag))
            {
                ...
                //because lots of problems with xamarin forms.
                //throw new BasicBlankException("Trying to register duplicate.  Rethink");
            }
            thisResults.Tag = tag;
        ...
        public T ReplaceObject<T>()
        {
            ...
            ReplaceObject(output);
            return output;
            //return (T) GetInstance(type, true);
        }
        ...
        //for now, no tag,  if i need tag, put in there later.
        public Type? LookUpType<T>()
        {
            ...
            if (possResults.Count == 0)
            {
                ...
                //otherwise, requires 2 calls.
                //throw new BasicBlankException($"Nothing was registered with the lookup of {type.Name}");
            }
