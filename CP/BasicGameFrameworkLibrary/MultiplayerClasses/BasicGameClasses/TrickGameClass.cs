...
namespace BasicGameFrameworkLibrary.MultiplayerClasses.BasicGameClasses
{
    ...
    {
        private readonly ITrickData _trickData; //risk for overflows.
        private readonly ITrickPlay _trickPlay; //risk for overflows;
        //if others need the trick stuff, could put to container (?)
        ...
        //something else has to enable the trick areas.  the main view model is responsible for it.
        ...
        public async Task CardClickedAsync() //this is a card being clicked on.
        {
            ...
            if (_model.PlayerHand1!.AutoSelect == HandObservable<T>.EnumAutoType.SelectAsMany)
            {
                if (_model.PlayerHand1.HowManySelectedObjects > 1)
                {
                    await UIPlatform.ShowMessageAsync("Can you only choose one card at a time to play");
            ...
            {
                await UIPlatform.ShowMessageAsync(PlayErrorMessage);
