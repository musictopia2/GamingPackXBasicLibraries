...
namespace BasicGamingUIXFLibrary.BasicControls.SolitaireClasses
{
    public class SolitaireOverlapLayoutXF : Layout<DeckOfCardsXF<SolitaireCard>>
    {
        ...
        internal static double MinWidth { get; set; } // i think this should be shared.  because cards are not going to be different sizes.
        ...
        public bool HasCard(SolitaireCard card)
        {
            foreach (var thisDeck in Children)
            {
                SolitaireCard other = (SolitaireCard)thisDeck.BindingContext;
                if (other.Deck == card.Deck)
                {
                    return true;
                }
            }
