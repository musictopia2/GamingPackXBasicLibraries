...
using BasicGamingUIXFLibrary.BasicControls.Layouts;
...
namespace BasicGamingUIXFLibrary.BasicControls.GameFrames
{
        ...
        where GW : BaseDeckGraphicsXF<CA, GC>, new()
    {
        ...
        public static readonly BindableProperty HandTypeProperty = BindableProperty.Create(propertyName: "HandType", returnType: typeof(HandObservable<CA>.EnumHandList), defaultValue: HandObservable<CA>.EnumHandList.Horizontal, declaringType: typeof(BaseHandXF<CA, GC, GW>), defaultBindingMode: BindingMode.TwoWay, propertyChanged: HandTypePropertyChanged);
        ...
        {
            if (HandType == HandObservable<CA>.EnumHandList.Vertical)
        ...
        private void ObjectList_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                ...
                //if (e.NewItems.Count == ObjectList!.Count)
                //{

                //    int oldCount;
                //    if (HasAngles == false)
                //    {
                //        oldCount = _thisStack!.Children.Count;
                //    }
                //    else
                //    {
                //        oldCount = _otherStack!.Children.Count;
                //    }
                //    if (oldCount == ObjectList.Count)
                //    {
                //        return;
                //    }
                //}
                ...
                foreach (var thisItem in e.NewItems)
                {
                    ...
                    if (y == 1)
                    {
                        GW? newControl = FindControl(newCard);
                        if (newControl != null)
                        {
                            return;
                        }
        ...
        {
            Binding ThisBind = new Binding(nameof(HandObservable<CA>.Visible));
