...
namespace BaseGPXPagesAndControlsXF.BasicControls.TrickUIs
{
            ...
            where PA : IPlayerTrick<SU, TC>, new()
    {
        ...
        public void Init(BasicTrickAreaViewModel<SU, TC> thisMod, IMultiplayerTrick<SU, TC, PA> others, string tagUsed)
        {
            ...
            foreach (var thisPrivate in privateList)
            {
                ...
                thisCard.SetBinding(BaseDeckGraphicsXF<TC, GC>.CommandProperty, thisBind); //i think
            ...
            Binding finalBind = new Binding(nameof(BasicTrickAreaViewModel<SU, TC>.Visible));
        ...
        private Label GetLabel(string text, TrickCoordinate thisView)
        {
            ...
            if (!string.IsNullOrEmpty(thisView.Text))
            {
                ...
                thisLabel.SetBinding(Label.IsVisibleProperty, ThisBind); // to hook up so we know if its visible or not.
            }
            else
                thisLabel.Text = text;
            return thisLabel;
