...
namespace BaseGPXPagesAndControlsXF.BasicControls.SingleCardFrames
{
    ...
    {
        private DeckViewModel<CA>? _thisMod;
        ...
        public void UpdateDeck(DeckViewModel<CA> cardModel)
        {
            BindingContext = null;
            BindingContext = cardModel;
            _thisObject!.BindingContext = null;
            _thisObject.BindingContext = _thisMod!.CurrentDisplayCard;
        }
        public void Init(DeckViewModel<CA> cardModel, string tagUsed) //risk with no property change (?)
        {
            BindingContext = cardModel;
            _tagUsed = tagUsed;
            _thisMod = cardModel;
            ...
            SetBinding(TextProperty, new Binding(nameof(DeckViewModel<CA>.TextToAppear)));
            SetBinding(IsEnabledProperty, new Binding(nameof(DeckViewModel<CA>.IsEnabled)));
            _thisGrid = new Grid();
            _thisObject = new GW();
            CA temps = new CA();
            ...
            Binding thisBind = new Binding(nameof(DeckViewModel<CA>.DeckObjectClickedCommand));
            ...
            SetBinding(IsVisibleProperty, new Binding(nameof(DeckViewModel<CA>.Visible)));
        ...
        private async Task PrepareToAnimateAsync(AnimateCardInPileEventModel<CA> data) // don't need index ever for this.
        {
            ...
            if (_thisMod!.DeckStyle == DeckViewModel<CA>.EnumStyleType.Unknown)
