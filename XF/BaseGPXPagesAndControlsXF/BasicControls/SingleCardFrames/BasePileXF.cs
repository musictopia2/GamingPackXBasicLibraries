...
namespace BaseGPXPagesAndControlsXF.BasicControls.SingleCardFrames
{
        ...
        where GW : BaseDeckGraphicsXF<CA, GC>, new()
    {
        ...
        public void Init(PileViewModel<CA> cardModel, string tagUsed)
        {
            ...
            _thisMod = cardModel;
            SetBinding(TextProperty, new Binding(nameof(PileViewModel<CA>.Text))); // hopefully the defaults from bindings will appear here.
            SetBinding(IsEnabledProperty, new Binding(nameof(PileViewModel<CA>.IsEnabled)));
            thisGrid = new Grid();
            _thisObject = new GW();
            _tagUsed = tagUsed;
            CA temps = new CA();
